...
def login():
    ...
    login_popup_button = driver.find_element(
        by=By.CSS_SELECTOR, value="button[data-test='have-account']"
    )
    login_popup_button.click()
    # Fill username
    username_input = driver.find_element(
        by=By.CSS_SELECTOR, value="input[data-test='email-input']"
    )
    username_input.send_keys(os.getenv("DUOLINGO_USERNAME"))
    # Fill password
    password_input = driver.find_element(
        by=By.CSS_SELECTOR, value="input[data-test='password-input']"
    )
    password_input.send_keys(os.getenv("DUOLINGO_PASSWORD"))
    # Click "Log in" button
    login_button = driver.find_element(
        by=By.CSS_SELECTOR, value="button[data-test='register-button']"
    )
    login_button.click()
        ...
        response = session.get(
            ...
            headers={
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0) \
                    ...
                    Chrome/109.0.0.0 Safari/537.36 Edg/109.0.1474.0",
            },
        )
...
def get_activities(after=date.today()):
    ...
    for activity in user_info["calendar"]:
        if (
            activity["datetime"]
            > datetime.fromisoformat(after.isoformat()).timestamp() * 1000
        ):
            activities.append(
                {
