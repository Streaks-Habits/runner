...
def get_access_token():
	client = OAuth2Session(os.getenv('STRAVA_APP_CLIENT_ID'), os.getenv('STRAVA_APP_CLIENT_SECRET'), scope='activity:read_all', redirect_uri='http://localhost')
...
def refresh_access_token(refresh_token):
	# Refresh token
	client = OAuth2Session(os.getenv('STRAVA_APP_CLIENT_ID'), os.getenv('STRAVA_APP_CLIENT_SECRET'), scope='activity:read_all', redirect_uri='http://localhost', token_endpoint=TOKEN_ENDPOINT)
	token = client.refresh_token(TOKEN_ENDPOINT, refresh_token=refresh_token, client_id=os.getenv('STRAVA_APP_CLIENT_ID'), client_secret=os.getenv('STRAVA_APP_CLIENT_SECRET'))
	return token['access_token'], token['refresh_token']
