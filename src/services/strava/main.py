...
from services.strava.api import ATHLETE_ACTIVITIES_ENDPOINT, refresh_and_get_access_token
...
def is_success(settings, day):
	...
	if day['type'] not in settings['activities'] and len(settings['activities']) > 0 and 'All' not in settings['activities']:
		return False
	# Check if there is a validation measure
	if not 'validation_measure' in settings:
		return True
	# If there is a validation measure, check if the measure is greater than the validation measure
