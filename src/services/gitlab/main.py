...
def get_events(after=date.today(), per_page=20):
	...
	page = 1
	while True:
		api_avents = req(f"{URL}/api/v4/events?after={after}&page={page}&per_page={per_page}")
		if len(api_avents) == 0:
			break
...
def is_success(settings, day):
	...
	if 'project_visibility' in settings and settings['project_visibility'] != day['project_visibility']:
		return False
	# Check if the action is in the list of actions
	if 'actions' in settings:
		if day['action'] not in settings['actions'] and len(settings['actions']) > 0 and 'All' not in settings['actions']:
			return False
	return True
